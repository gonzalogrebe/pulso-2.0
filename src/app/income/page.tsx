...
export default function IncomePage() {
  ...
  const fetchAllTransactions = async () => {
    try {
      ...
      console.log('Respuesta de la API en frontend (IncomePage):', data);

      if (!Array.isArray(data)) {
        throw new Error('La respuesta de la API no es un array de transacciones');
      }

      // Filtrar transacciones de tipo "Ingreso"
      const incomeTransactions = data.filter((tx: Transaction) => tx.tipo.toLowerCase() === 'ingresos' || tx.tipo.toLowerCase() === 'ingreso');

      setTransactions(incomeTransactions);
      setTotal(incomeTransactions.length);
    } catch (error) {
      console.error('Error en fetchAllTransactions (IncomePage):', error);
      setTransactions([]);
      setTotal(0);
  ...
  const calculateYearSummary = (transactions: Transaction[]) => {
    ...
    console.log('Resumen Anual (IncomePage):', {
      total: {
        budget: totalBudget,
        actual: totalActual
      },
  ...
  const calculateMonthSummary = (transactions: Transaction[]) => {
    ...
    console.log('Resumen Mensual (IncomePage):', {
      total: {
        budget: totalBudget,
        actual: totalActual
      },
  ...
  return (
    <MUI.Box sx={{ p: 3 }}>
      ...
      <MUI.Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <MUI.Typography variant="h4">Ingresos</MUI.Typography>
      </MUI.Box>
                  ...
                  Object.entries(monthSummary.categories || {}).map(([categoria, data]) => {
                    ...
                    return (
                      <MUI.TableRow key={categoria} hover>
                        ...
                        <MUI.TableCell align="right" sx={{ color: variance > 0 ? 'success.main' : 'error.main' }}>
